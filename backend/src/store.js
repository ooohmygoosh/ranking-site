...
function submissionPlacements(submission) {
  return Array.isArray(submission.placements) ? submission.placements : [];
}

function submissionDeleteIds(submission) {
  return Array.isArray(submission.deleteItemIds) ? submission.deleteItemIds : [];
}

function settleList(data, listId, nowIso = new Date().toISOString(), options = {}) {
  const list = data.rankLists.find((item) => item.id === listId);
  if (!list) return;
  ...
  list.items = list.items.filter((item) => {
    const deleteCount = submissions.filter((submission) => submissionDeleteIds(submission).includes(item.id)).length;
    if (options.forceDelete) return deleteCount === 0;
  ...
  list.items.forEach((item) => {
    const placements = submissions
      .map((submission) => submissionPlacements(submission).find((placement) => placement.itemId === item.id))
      .filter(Boolean);
...
function settleListNow(id) {
  ...
  const list = data.rankLists.find((item) => item.id === id);
  if (!list) return null;
  settleList(data, id, new Date().toISOString(), { forceDelete: true });
  writeStore(data);
...
function getListSummary(listId) {
  ...
  list.items.forEach((item) => {
    ...
    pendingSubmissions.forEach((submission) => {
      if (submissionDeleteIds(submission).includes(item.id)) deleteCount += 1;
      const placement = submissionPlacements(submission).find((entry) => entry.itemId === item.id);
